...
![集群元数据及状态协调](/img/lindb/design/coordinator.png)
...
- 每个 Storage Cluster 会 Watch Storage 节点的存活情况，并把该 Storage 整体的状态信息写到 **/state/storage/cluster/\{cluster name\}** 下以供 [Storage Cluster Status State Machine](/docs/lindb/design/coordinator#storage-status) 使用；
...
- Watch KEY 里面的信息由 Master 统一写入，Master 会 Watch 每个 Storage 集群各节点的存活情况，并把最终集群的信息写入到 Watch KEY 下面, 具体请看 [Storage Live Node State Machine](/docs/lindb/design/coordinator#storage-live-node)；
