...
- `Watch KEY` 里面的信息由 `Master` 统一写入，`Master` 会 Watch 每个 `Storage` 集群各节点的存活情况，并把最终集群的信息写入到 `Watch KEY` 下面, 具体请看 [Storage Live Node State Machine](./coordinator.html#storage-live-node) ；
