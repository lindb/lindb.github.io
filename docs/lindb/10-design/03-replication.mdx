...
![Databae storage shard](/img/lindb/design/storage_database.png)
...
![Local replication](/img/lindb/design/local_replication.png)
...
In fact, after converting to **id**, the storage will be **cpu => 1(metric id), host=1.1.1.1 => 1(time series id), load => 1(field id)**, simplified to **1 1 1514214168614 =>1**, please refer to [inverted index](/docs/lindb/design/inverted-index) for the specific index structure;
...
- When obtained **ID**, data will be written into storage uint, this process is similar to **LSM**, which writes data into the memory database firstly, and directly write to the memory to achieve high throughput performance. After the size of memory data reaches the memory limit, the **Flush** operation will be triggered. For details, please refer to [Storage Format](/docs/lindb/design/storage), [Memory Database](/docs/lindb/design/memory-database);
...
![Remote replication](/img/lindb/design/remote_replication.png)
...
1. [_bigqueue_](https://github.com/bulldog2011/bigqueue)
