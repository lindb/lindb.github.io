...
可以在 [config/standalone.toml.example](https://github.com/lindb/lindb/tree/main/config/standalone.toml.example) 找到默认值的配置文件，重命名为 standalone.toml 即可作为 **Standalone** 启动参数文件。Standalone 作为 LinDB 一种特殊的单机模式，其内部集成了 Broker/Storage/ETCD 等组件，其为 [Broker](/docs/lindb/reference/configuration#broker)/[Storage](/docs/lindb/reference/configuration#storage) 参数如上，这里只介绍不包含 Broker/Storage 这部分参数说明。
