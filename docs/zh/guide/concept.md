...
:::tip 尽量不要使用不可枚举的字符串作为指标名或者标签，如 UUID、用户 ID 等。
:::
...
:::tip 数据聚合指某个时间窗口，如 5/10 秒、1 分钟内的值，如 1 分钟内的总和可以用 Sum。
:::
...
:::tip Histogram 作为复杂类型运行广泛，同时也内置了 Min/Max/Sum 等统计值，意味着存储计算 Histogram 需要更多的服务器资源。
:::